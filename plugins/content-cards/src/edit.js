							...
							>
								...
								<div style={{marginBottom: '20px', padding: '15px', background: '#fff', borderRadius: '4px'}}>
									...
									<MediaUploadCheck>
										<MediaUpload
											onSelect={(media) =>
												updateCard(index, null, { img: media.url, img_alt: media.alt, img_title: media.title?.rendered || media.title || '' })
											}
											type="image"
											allowedTypes={['image']}
											value={card.img}
											render={({ open }) => (
												<div style={{marginBottom: '10px'}}>
													{card.img && (
														<>
															<p style={{fontSize:'12px', margin: '5px 0'}}>{__('Alt Text:')} {card.img_alt || card.img_title}</p>
															<Button
																isLink
																isDestructive
																onClick={() => updateCard(index, null, {img: '', img_alt: '', img_title: ''})}
															>
																{__('Remove Image')}
															</Button>
														</>
